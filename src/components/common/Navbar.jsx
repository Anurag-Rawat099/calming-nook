...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50">
      ...
      {/* Mobile Menu */}
      {open && (
        <div className="absolute top-[60px] left-4 right-4 z-50 md:hidden theme-card rounded-2xl p-5 shadow-xl">
          <div className="flex flex-col">
            {navItems.map((item) => (
              <Link
                ...
                href={item.link}
                onClick={() => setOpen(false)}
                className="py-3 border-b border-black/5 text-sm font-medium transition-colors hover:text-[var(--primary)]"
              >
                {item.title}
              </Link>
            ))}
            ...
            <Link
              href="/contact"
              onClick={() => setOpen(false)}
              className="primary-btn mt-5 text-center w-full"
            >
              Book Stay
            </Link>
          </div>
